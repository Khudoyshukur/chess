...
namespace pblczero {
    ...
    public:
        ...
        class Smolgen : public lczero::ProtoMessage {
        public:

            bool has_compress() const { return has_compress_; }
            const Layer& compress() const { return compress_; }
            Layer* mutable_compress() {
                has_compress_ = true;
                return &compress_;
            }
        ...
        public:
            ...
            bool has_smolgen() const { return has_smolgen_; }
            const Smolgen& smolgen() const { return smolgen_; }
            Smolgen* mutable_smolgen() {
                has_smolgen_ = true;
                return &smolgen_;
            }
        ...
        bool has_ip_emb_w() const { return has_ip_emb_w_; }
        const Layer& ip_emb_w() const { return ip_emb_w_; }
        Layer* mutable_ip_emb_w() {
            has_ip_emb_w_ = true;
            return &ip_emb_w_;
        }

        bool has_ip_emb_b() const { return has_ip_emb_b_; }
        const Layer& ip_emb_b() const { return ip_emb_b_; }
        Layer* mutable_ip_emb_b() {
            has_ip_emb_b_ = true;
            return &ip_emb_b_;
        }

        bool has_ip_mult_gate() const { return has_ip_mult_gate_; }
        const Layer& ip_mult_gate() const { return ip_mult_gate_; }
        Layer* mutable_ip_mult_gate() {
            has_ip_mult_gate_ = true;
            return &ip_mult_gate_;
        }

        bool has_ip_add_gate() const { return has_ip_add_gate_; }
        const Layer& ip_add_gate() const { return ip_add_gate_; }
        Layer* mutable_ip_add_gate() {
            has_ip_add_gate_ = true;
            return &ip_add_gate_;
        }

        EncoderLayer* add_encoder() { return &encoder_.emplace_back(); }
        const std::vector<EncoderLayer>& encoder() const { return encoder_; }
        const EncoderLayer& encoder(size_t idx) const { return encoder_[idx]; }
        size_t encoder_size() const { return encoder_.size(); }

        bool has_headcount() const { return has_headcount_; }
        std::uint32_t headcount() const { return headcount_; }
        ...
        bool has_ip_val_w() const { return has_ip_val_w_; }
        const Layer& ip_val_w() const { return ip_val_w_; }
        Layer* mutable_ip_val_w() {
            has_ip_val_w_ = true;
            return &ip_val_w_;
        }

        bool has_ip_val_b() const { return has_ip_val_b_; }
        const Layer& ip_val_b() const { return ip_val_b_; }
        Layer* mutable_ip_val_b() {
            has_ip_val_b_ = true;
            return &ip_val_b_;
        }
        ...
        bool has_ip_mov_w() const { return has_ip_mov_w_; }
        const Layer& ip_mov_w() const { return ip_mov_w_; }
        Layer* mutable_ip_mov_w() {
            has_ip_mov_w_ = true;
            return &ip_mov_w_;
        }

        bool has_ip_mov_b() const { return has_ip_mov_b_; }
        const Layer& ip_mov_b() const { return ip_mov_b_; }
        Layer* mutable_ip_mov_b() {
            has_ip_mov_b_ = true;
            return &ip_mov_b_;
        }
        ...
        bool has_smolgen_w() const { return has_smolgen_w_; }
        const Layer& smolgen_w() const { return smolgen_w_; }
        Layer* mutable_smolgen_w() {
            has_smolgen_w_ = true;
            return &smolgen_w_;
        }

        bool has_smolgen_b() const { return has_smolgen_b_; }
        const Layer& smolgen_b() const { return smolgen_b_; }
        Layer* mutable_smolgen_b() {
            has_smolgen_b_ = true;
            return &smolgen_b_;
        }
    ...
    public:
        ...
        static std::string NetworkStructure_Name(NetworkStructure val) {
            switch (val) {
                ...
                case NETWORK_ONNX:
                    return "NETWORK_ONNX";
                case NETWORK_ATTENTIONBODY_WITH_HEADFORMAT:
        ...
        bool has_smolgen_activation() const { return has_smolgen_activation_; }
        ActivationFunction smolgen_activation() const { return smolgen_activation_; }
        void set_smolgen_activation(ActivationFunction val) {
            has_smolgen_activation_ = true;
            smolgen_activation_ = val;
        }

        bool has_ffn_activation() const { return has_ffn_activation_; }
        ActivationFunction ffn_activation() const { return ffn_activation_; }
        void set_ffn_activation(ActivationFunction val) {
    ...
    private:
        void SetVarInt(int field_id, std::uint64_t val) override {
            switch (field_id) {
                ...
                case 8: set_smolgen_activation(static_cast<ActivationFunction>(val)); break;
                case 9: set_ffn_activation(static_cast<ActivationFunction>(val)); break;
            }
        }
